...
### Sprint 5 – Testing, Finalization and Deployment

The fifth sprint focused on translating the validated benchmark from Sprint 4 into a fully operational inference system and deployable web application. The work proceeded along three interconnected directions: selecting and calibrating the production model, assembling the end-to-end inference pipeline, and building the interactive clinical interface that exposes the entire system to end users.

All eight model configurations produced during the benchmark were made available in the deployed application, allowing users to switch between them at runtime and compare predictions under different architectures and training conditions. In addition to these eight checkpoints, a ninth configuration was registered as the default: a variant of `EfficientNet-B0 | base_224x224` in which the raw output probabilities were adjusted via Platt Scaling to improve calibration on the imbalanced test distribution, achieving a test AUC of `0.9015` and sensitivity of `90.42%`. The main effort of this sprint, however, was the design and deployment of the application itself.

The inference pipeline was implemented in a modular `SkinCancerPredictor` class that encapsulates all preprocessing and prediction steps. Starting from a raw image upload, the pipeline optionally applies hair artifact removal via a black-hat morphological operation followed by Telea inpainting, then passes the image through the U-Net segmentation model to obtain a binary lesion mask. The U-Net was trained from scratch on the HAM10000 segmentation masks using an encoder–decoder architecture with four resolution levels (`64 → 32 → 16 → 8 → 4` pixels), skip connections at each level, and `Dropout2D` regularization. Its input is resized to `64×64`, and the predicted mask is upscaled bilinearly back to the original image resolution. The mask is used to compute a tight bounding box around the lesion, which is expanded by a `15%` margin, cropped, and padded to a square using edge-replication before being resized to the classifier input resolution of `224×224`. If the mask is empty or unavailable, the pipeline falls back gracefully to full-frame square padding. In addition to the three-zone classification decision, the pipeline computes two explainability overlays: Grad-CAM gradients with respect to the `conv_head` layer of EfficientNet-B0, producing a spatial attention heatmap, and an occlusion sensitivity map generated by systematically replacing `40×40` pixel patches with the mean local color and measuring the drop in predicted melanoma probability. From the segmentation mask the pipeline also derives quantitative proxies for the ABCDE dermoscopic criteria — asymmetry from the intersection-over-union under horizontal and vertical reflection, border irregularity from the circularity ratio of the mask contour, color from the standard deviation of pixel intensities within the lesion region, and relative diameter as a pixel-level proxy normalized by the image diagonal.

The primary deliverable of Sprint 5 is the *Dermoscopy Triage Studio*, a multi-tab Streamlit application that exposes the full pipeline through an interactive web interface. A central design decision was the implementation of a live pipeline preview: rather than blocking until inference completes, a callback `on_step(idx, image)` is invoked at each preprocessing stage, allowing the frontend to update placeholder elements in real time so the user observes each transformation as it occurs. The interface is organized into three tabs. The *Resultado* tab presents the original image alongside a three-image pipeline strip, the three-zone risk indicator with a visual probability bar, a composite ABCDE score, and a structured facts grid with lesion coverage metrics. The *Pipeline* tab shows all six intermediate images produced during preprocessing, each annotated with a step description. The *Modelo* tab contains a comparative table of all eight trained configurations, with columns for AUC, sensitivity, specificity, critical false-negative counts — melanomas classified as low risk, representing the most dangerous error — uncertain-zone false negatives, total false negatives, and zone-level melanoma capture rates, formatted with inline bar charts and a green highlight for the deployed configuration. When an uploaded filename matches a HAM10000 image identifier, the application retrieves the ground truth label and computes a TP/TN/FP/FN outcome displayed as a color-coded badge. A visually similar case retrieval mechanism is also integrated, using cosine similarity over normalized pixel embeddings computed for all images in the treated manifest. The application supports switching between all nine model configurations at runtime; changing the active model or the hair removal toggle invalidates the session cache and triggers reprocessing of all uploaded images under the new settings, enabling controlled within-session comparisons. Results can be exported individually as structured JSON reports or collectively as a CSV session summary.

The production instance was deployed on an AWS EC2 virtual machine running Ubuntu, with the application's dependencies installed in a Python virtual environment and the repository synchronized directly from GitHub. Streamlit was configured to bind exclusively to the loopback interface (`--server.address localhost`) to prevent unauthenticated public access. Remote access is provided through SSH port forwarding (`-L 8501:localhost:8501`), tunneling the application to the operator's local browser without requiring firewall changes or a reverse proxy. The Streamlit process is managed by a persistent `tmux` session to ensure availability across SSH disconnections.

In summary, Sprint 5 completed the transition from a controlled benchmark to a fully functional and deployed clinical triage system. All nine model configurations are available at runtime, with the calibrated EfficientNet-B0 serving as the default entry point. The application demonstrates the complete research-to-deployment lifecycle: dataset-driven preprocessing, systematic benchmark across eight configurations, and an interactive explainable interface accessible through a cloud-hosted EC2 instance.
